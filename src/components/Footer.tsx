...
import { MapPin, Phone, Clock, ShieldCheck, FileText } from 'lucide-react';
import { useLanguage } from '../i18n/LanguageContext';
import type { Lang } from '../i18n/translations';
import LegalModal from './LegalModal';

const legalLabels: Record<Lang, { legal: string; privacy: string }> = {
  fr: { legal: 'Mentions légales', privacy: 'Politique de confidentialité' },
  en: { legal: 'Legal notice', privacy: 'Privacy policy' },
  ko: { legal: '법적 고지', privacy: '개인정보 처리방침' },
  zh: { legal: '法律声明', privacy: '隐私政策' },
};

const Footer = () => {
  const { t, lang } = useLanguage();
  const [legalOpen, setLegalOpen] = useState<null | 'legal' | 'privacy'>(null);

  return (
    <>
      <footer id="contact" className="bg-[#050505] text-white pt-24 pb-44 lg:pb-12 border-t border-[#D4AF37]/10 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-16">
            <div className="flex flex-col items-center md:items-start">
              <div className="relative mb-6 rounded-2xl border border-[#D4AF37]/25 bg-[#050505] px-3 py-2 shadow-[inset_0_0_0_1px_rgba(255,255,255,0.04)]">
                <img src="/images/seoul-logo.png" alt="Seoul" className="relative h-14 w-auto object-contain rounded-xl" />
              </div>
              <p className="text-gray-500 mb-8 text-center md:text-left leading-relaxed font-light text-sm max-w-sm">
                {t('footer_desc')}
              </p>
            ...
            <div className="flex flex-col items-center md:items-start">
              <h3 className="text-xs font-light mb-8 uppercase tracking-[0.3em] text-white/40">{t('footer_info')}</h3>
          ...
          <div className="border-t border-white/[0.05] pt-8 text-center space-y-4">
            <p className="text-[#D4AF37]/30 text-[11px] font-light tracking-[0.3em] uppercase">
              {t('footer_tagline')}
            </p>

            <div className="flex flex-col sm:flex-row justify-center items-center gap-3 sm:gap-5 mb-2">
              <button
                type="button"
                onClick={() => setLegalOpen('legal')}
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-full border border-white/10 bg-white/[0.02] px-4 py-3 text-[11px] uppercase tracking-[0.18em] text-white/55 hover:border-[#D4AF37]/30 hover:text-[#D4AF37] transition-colors"
              >
                <FileText size={14} />
                {legalLabels[lang].legal}
              </button>
              <button
                type="button"
